...
import { jobScopeLabel } from "@/services/connector-queries.service";
...
function ConnectorJobsCard({ tenantId, companyId }: { tenantId: string; companyId: string }) {
  const panel = useQuery({ queryKey: ["connector-initial-loads", tenantId, companyId], queryFn: () => connectorInitialLoadsService.list(companyId || undefined), enabled: !!tenantId, refetchInterval: query => (query.state.data?.summary.pending ?? 0) > 0 ? 3000 : 10000, refetchIntervalInBackground: true });
  ...
  return <Card className="p-5">
    ...
    <div className="mt-3 flex flex-wrap gap-3"><Button variant={data.companies.length===0?"primary":"secondary"} onClick={()=>bootstrap.mutate()} disabled={!connectorId||bootstrap.isPending}><RefreshCw size={16} className={bootstrap.isPending?"animate-spin":""}/>{bootstrap.isPending?"Solicitando ao Connector...":data.companies.length===0?"Atualizar / tentar novamente":"Atualizar empresas do Consinco"}</Button>{data.companies.length>0&&<Button onClick={()=>start.mutate()} disabled={!connectorId||!effectiveCompanyId||start.isPending}><Play size={16}/>{start.isPending?"Iniciando...":"Iniciar carga"}</Button>}{activeLoad && !["APPROVED","PENDING_VALIDATION"].includes(activeLoad.status) && <Button variant="secondary" onClick={()=>resume.mutate(activeLoad.id)} disabled={resume.isPending}><RefreshCw size={16}/>{resume.isPending?"Retomando...":"Retomar carga"}</Button>}{activeLoad && activeLoad.status==="PENDING_VALIDATION" && <><Button onClick={()=>approve.mutate(activeLoad.id)} disabled={approve.isPending}><Check size={16}/>{approve.isPending?"Aprovando...":"Aprovar carga"}</Button><Button variant="secondary" onClick={()=>{const reason=window.prompt("Motivo da rejeição (opcional, mas recomendado):");if(reason===null)return;reject.mutate({loadId:activeLoad.id,reason:reason.trim()||undefined});}} disabled={reject.isPending}><X size={16}/>{reject.isPending?"Rejeitando...":"Rejeitar carga"}</Button></>}{activeLoad && <Button variant="danger" onClick={()=>setResetOpen(open=>!open)}><Trash2 size={16}/>Zerar carga</Button>}</div>
    {activeLoad && activeLoad.status==="PENDING_VALIDATION" && <p className="mt-3 rounded-lg bg-cyan-50 p-3 text-sm text-cyan-800">Todos os jobs desta carga terminaram e aguardam validação. Aprove (ou rejeite) para liberar uma nova execução — inclusive se novas consultas foram adicionadas depois desta carga, elas só entram após aprovar e clicar em “Iniciar carga” de novo.</p>}
    {resetOpen && activeLoad && <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-4">
      <p className="text-sm font-semibold text-red-800">Zerar a carga inicial de {data.companies.find(company=>company.id===activeLoad.companyId)?.tradeName || "empresa selecionada"}</p>
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-semibold text-red-800">Sua senha de login<input type="password" value={resetPassword} onChange={event=>setResetPassword(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-red-300 bg-white px-3 text-sm font-normal" autoComplete="current-password" /></label>
        <label className="text-sm font-semibold text-red-800">Motivo (opcional)<input value={resetReason} onChange={event=>setResetReason(event.target.value)} className="mt-1.5 h-10 w-full rounded-md border border-red-300 bg-white px-3 text-sm font-normal" placeholder="Ex.: carga transmitida em versão anterior" /></label>
      </div>
      ...
      {reset.isError&&<div className="mt-3"><ErrorState message={getApiErrorMessage(reset.error)}/></div>}
    </div>}
    {reset.isSuccess&&<p className="mt-3 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-800">Carga zerada: {reset.data.deleted.records} registro(s), {reset.data.deleted.batches} lote(s) e {reset.data.deleted.jobs} job(s) removidos. Clique em “Iniciar carga” para gerar tudo de novo.</p>}
    {bootstrap.isSuccess&&<p className="mt-3 rounded-lg bg-cyan-50 p-3 text-sm text-cyan-800">Solicitação enviada. O Connector busca automaticamente a consulta de empresas, mesmo sem empresa ou carga inicial cadastrada. Esta tela será atualizada durante o processamento.</p>}
