...
export type FiscalFinding = { code: string; severity: "HIGH" | "MEDIUM" | "LOW"; reason: string; observation: string; estimatedOverpayment?: number; estimatedOverpaymentItemCount?: number };
...
export const fiscalComplianceService = {
  products: async () => (await api.get<ApiResponse<{ requiresCompanySelection: boolean; client: { name: string; documentRoot: string; state: string | null; branches: number } | null; items: FiscalProduct[] }>>("/fiscal-compliance/products",{timeout:60_000})).data.data,
