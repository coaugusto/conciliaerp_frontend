...
export type AdjustmentProposalItemInput = { itemCode: string; title: string; origin: AdjustmentOrigin; affected: number; estimatedHours: number; hourlyRate: number; scheduledDate?: string };
...
export const adjustmentSuggestionsService = {
  ...
  upsertPricing: async (input: { itemCode: string; estimatedHours: number; hourlyRate: number }) =>
